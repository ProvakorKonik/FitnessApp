package com.stayfit.myapplication;
...
import com.stayfit.myapplication.View.DayMain;
import com.stayfit.myapplication.View.Profile;
...
public class LoginRegistration extends AppCompatActivity {
    ...
    private void UploadCropedImageFunction(Uri filePath) {
        if(!dRetrivePhotoURL.equals("NO")){
            ...
            ref = storageReference.child("StayFit/Users_Profile_Pic/"+dUserUID+"/"+ dUserUID +"."+getFileExtention(filePath));
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                ...
                                public void onSuccess(Uri uri) {
                                    ...
                                    note.put("height",dUserPhone);;
                                    note.put("userType",dUserType);;
                                    note.put("points",0);   //intger coins        new
                                    note.put("tried",0);  //integer
                                    note.put("practice_days",0);  //integer
                                    note.put("exerciseTime",0);  //integer
                                    note.put("weight",Integer.parseInt(dUserBio));  //Intger
                                    //note.put("gender", dGender);
                                    //note.put("type","NORMAL");
                                    //note.put("follower","0");
                                    //note.put("post","0");
                                    //note.put("reg_date",dUserRegistrationDate);
                                    db.collection("StayFit").document("Info").collection("Normal_USER").document(dUserUID).set(note)
                                            .addOnSuccessListener(new OnSuccessListener<Void>() {
                                                @Override
                                                public void onSuccess(Void aVoid) {
                                                    Toast.makeText(getApplicationContext(),"Successfully Uploaded", Toast.LENGTH_SHORT).show();
                                                    ...
                                                    mUserInfoPhoneNo.setText("");
                                                    finish();
                                                    Intent intent = new Intent(LoginRegistration.this,DayMain.class);
                                                    intent.setFlags(intent.getFlags() | Intent.FLAG_ACTIVITY_NO_HISTORY);
                                                    startActivity(intent);
